...
const inject = (target) => {
    ...
    setInterval(() => {
        target._opbots.mousePosition.x = window.application.getActiveTab().cursorX; 
        target._opbots.mousePosition.y = window.application.getActiveTab().cursorY;
        target._opbots.gameData(2);
        if(window.application.ghostCells && window.application.ghostCells[0]) {
            server.sendCheck(window.application.ghostCells[0].x, window.application.ghostCells[0].y);
        } else {
            const tab = window.application.getActiveTab()
            server.sendCheck(
                tab.serverX(tab.playerX)
                , tab.serverY(tab.playerY)
                );
        }
    }, 100);

    window.application.master.on('offset',(tab) => {
...
let injectLogo = document.createElement("input");
    ...
    injectLogo.value = 'https://i.imgur.com/gO0SJq3.png'
